...
function Card({product}){
    const {addProduct,onSetProduct, openIsDetailOpen} = useContext(ProductsContext);
    ...
    return (
        <div className="flex-1 bg-white cursor-pointer w-56 h-60 rounded-lg" onClick={()=>onClickProduct(product)}>
            <figure className="relative mb-2 w-full h-4/5">
                <span className="absolute bottom-0 left-0 bg-white/60 rounded-lg text-black text-xs m-2 px-2 py-0.5">
                    {product.category.name}</span>
                <img className="w-full h-full object-cover rounded-lg" 
                 src={product.currentImage} alt={product.description} />
